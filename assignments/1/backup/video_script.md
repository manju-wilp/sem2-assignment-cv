...
## Chethan: Introduction & Problem Statement (~45 sec)

> "Hi, we are Group 12 and today we'll present our solution for Problem 2 — Lane Detection using Classical Computer Vision and Machine Learning.
>
...
>
> We tested on 6 diverse images — a night highway, daylight roads with buildings and trees, multi-lane highways with traffic, and a challenging scene with a large tow truck. Let me hand over to Jayasurya who'll walk through the preprocessing."

---

## Jayasurya: Preprocessing & Edge Detection (~45 sec)
...
>
> For edge detection, we use the Canny algorithm with thresholds 50 and 150, following the standard 1:3 ratio. As you can see in the notebook, the Canny output clearly picks up lane edges across all lighting conditions. Aswathi will explain the ROI and Hough transform."

---

## Aswathi: ROI, Hough Transform & Slope Classification (~50 sec)
...
>
> Our results show that for clean images like image2 and image3, all three methods produce identical results. But for the cluttered image5, RANSAC gives a different slope — minus 0.45 versus minus 0.40 — showing its outlier rejection in action. Overall, RANSAC is the most robust method. Manju will present the results and limitations."

---

## Manju: Results, Limitations & Conclusion (~50 sec)
...
- **Key visuals to show** while speaking:
  - Chethan: Dataset overview grid (all 6 images)
  - Jayasurya: Preprocessing pipeline figure (show for 2–3 images), Canny edge grid
  - Aswathi: ROI boundary + masked edges, Hough segments grid
  - Vipul: ML fitting results table, method comparison side-by-side (3 methods on one image)
  - Manju: Pipeline results grid (all 6 images), metrics bar charts, analysis markdown
